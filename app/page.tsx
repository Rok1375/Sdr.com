...
export default function Portfolio() {
  ...
  useEffect(() => {
    ...
    // Custom Cursor & Blob Mask with Smooth Interpolation (GSAP quickTo)
    let xToCursor: gsap.QuickToFunc;
    let yToCursor: gsap.QuickToFunc;
    
    // We use a custom requestAnimationFrame loop for the CSS variables to ensure buttery smooth mask trailing
    ...
    if (cursorRef.current) {
      xToCursor = gsap.quickTo(cursorRef.current, "x", { duration: 0.15, ease: "power3.out" });
      yToCursor = gsap.quickTo(cursorRef.current, "y", { duration: 0.15, ease: "power3.out" });
    }
    ...
    const renderLoop = () => {
      // Lerp for the gooey blob mask trailing effect
      const ease = 0.08;
  ...
  return (
    <div className="min-h-screen relative overflow-hidden">
      ...
      <header className="fixed bottom-0 left-0 w-full z-[100] px-6 py-6 md:px-16 md:py-8 flex justify-between items-center mix-blend-difference pointer-events-none">
      ...
      {/* ========== TICKER SECTION ========== */}
      <section id="ticker" className="h-[30vh] md:h-screen flex items-center overflow-hidden border-y border-white/5" role="marquee" aria-label="Brand statement ticker">
      ...
      <section id="testimonials" className="py-20 md:py-32 px-6 md:px-8" role="region" aria-label="Client testimonials">
